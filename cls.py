...
def _init_(args):
    ...
    if not os.path.exists(os.path.join(args.root, args.model)):
        os.makedirs(os.path.join(args.root, args.model))
    if not os.path.exists(os.path.join(args.root, args.model, 'checkpoints')):
        os.makedirs(os.path.join(args.root, args.model, 'checkpoints'))
...
def train(args, io):
    ...
    for epoch in range(args.epochs):
        ...
        outstr = 'Epoch %d/%d - \033[31mtraining loss: %.6f\033[0m, training acc: %.6f, training avg acc: %.6f, cost totally %ds.' % (
            epoch + 1, args.epochs, train_loss * 1.0 / count,
            metrics.accuracy_score(train_true, train_pred),
            metrics.balanced_accuracy_score(train_true,
                                            train_pred), total_time)
        io.cprint(outstr)
        """
        Testing
        """
        ...
        avg_per_class_acc = metrics.balanced_accuracy_score(
            test_true, test_pred)
        outstr = 'Epoch %d/%d - testing loss: %.6f, \033[32mtesting acc: %.6f\033[0m, testing avg acc: %.6f, cost totally %ds' % (
            epoch + 1, args.epochs, test_loss * 1.0 / count, test_acc,
            avg_per_class_acc, total_time)
        io.cprint(outstr)
    ...
    io.cprint("Best accuracy %.6f in epoch %d" % (best_test_acc, best_epoch))
...
if __name__ == "__main__":
    ...
    io = Logger(os.path.join(args.root, args.model, 'run.log'))
    io.cprint(str(args))
